...
async def audit_cloud_run(client: GCPClient) -> list[Finding]:
    ...
    for svc in services:
        ...
        # Comprobar configuración de ingress del servicio (v1.3)
        # El ingress se define en la anotación run.googleapis.com/ingress
        _ingress_anotaciones = meta.get("annotations", {})
        _ingress_val = _ingress_anotaciones.get("run.googleapis.com/ingress", "")
        if _ingress_val == "all":
            findings.append(Finding(
                tool=TOOL_NAME, severity="HIGH",
                type="cloudrun-ingress-all",
                title=(
                    f"Cloud Run '{svc_name}' tiene ingress configurado "
                    "como 'all' (accesible desde internet)"
                ),
                description=(
                    f"El servicio Cloud Run '{svc_name}' tiene la anotación "
                    "'run.googleapis.com/ingress: all', lo que permite tráfico "
                    "entrante desde cualquier origen en internet. Combinado con "
                    "un IAM permisivo puede suponer acceso público no controlado."
                ),
                affected=svc_url or svc_fqn,
                recommendation=(
                    "Revisar si el servicio debe estar accesible desde internet. "
                    "Si es interno, cambiar el ingress a 'internal' o "
                    "'internal-and-cloud-load-balancing':\n"
                    f"  gcloud run services update {svc_name} --ingress internal"
                ),
                module="CloudRun",
            ))
        ...
        for binding in iam_data.get("bindings", []):
            ...
            if "allUsers" in members:
                ...
                # Hallazgo específico CLOUDRUN_PUBLIC_INVOKE cuando el rol es invoker (v1.3)
                if role == "roles/run.invoker":
                    findings.append(Finding(
                        tool=TOOL_NAME, severity="CRITICAL",
                        type="CLOUDRUN_PUBLIC_INVOKE",
                        title=(
                            f"Cloud Run '{svc_name}': allUsers tiene "
                            "roles/run.invoker — invocación pública sin autenticación"
                        ),
                        description=(
                            f"El servicio Cloud Run '{svc_name}' (URL: {svc_url}) "
                            "tiene el rol 'roles/run.invoker' asignado a 'allUsers'. "
                            "Cualquier usuario de internet puede invocar el servicio "
                            "directamente sin ningún tipo de autenticación."
                        ),
                        affected=svc_url or svc_fqn,
                        recommendation=(
                            "Revocar el binding allUsers/roles/run.invoker:\n"
                            f"  gcloud run services remove-iam-policy-binding {svc_name} \\\n"
                            "    --region=REGION \\\n"
                            "    --member='allUsers' --role='roles/run.invoker'\n"
                            "Implementar autenticación en la aplicación o usar "
                            "Identity-Aware Proxy para controlar el acceso."
                        ),
                        module="CloudRun",
                    ))
    ...
    for repo in repos:
        ...
        for binding in iam_data.get("bindings", []):
            ...
            if "allUsers" in members:
                ...
                # Hallazgo ARTIFACT_REGISTRY_PUBLIC unificado para visibilidad pública (v1.3)
                findings.append(Finding(
                    tool=TOOL_NAME, severity="CRITICAL",
                    type="ARTIFACT_REGISTRY_PUBLIC",
                    title=(
                        f"Artifact Registry '{repo_short}' visible públicamente "
                        f"(allUsers · {repo_format})"
                    ),
                    description=(
                        f"El repositorio de Artifact Registry '{repo_short}' "
                        f"(formato: {repo_format}) está accesible para 'allUsers' "
                        f"con el rol '{role}'. Las imágenes/artefactos almacenados "
                        "son descargables por cualquier usuario sin autenticación."
                    ),
                    affected=repo_name,
                    recommendation=(
                        "Eliminar el binding público y restringir el acceso a "
                        "service accounts o grupos específicos. Para repositorios "
                        "que deban ser públicos, considerar un registry dedicado "
                ...
                # Hallazgo ARTIFACT_REGISTRY_PUBLIC unificado para allAuthenticatedUsers (v1.3)
                findings.append(Finding(
                    tool=TOOL_NAME, severity="HIGH",
                    type="ARTIFACT_REGISTRY_PUBLIC",
                    title=(
                        f"Artifact Registry '{repo_short}' visible para todos "
                        f"los usuarios autenticados de Google (allAuthenticatedUsers)"
                    ),
                    description=(
                        f"El repositorio '{repo_short}' (formato: {repo_format}) "
                        "tiene visibilidad pública para cualquier cuenta de Google "
                        f"autenticada (rol: '{role}'). Los artefactos pueden ser "
                        "accedidos sin pertenecer a la organización."
                    ),
                    affected=repo_name,
                    recommendation=(
                        "Restringir el acceso del repositorio a service accounts "
                        "o grupos específicos de la organización."
                    ),
                    module="ArtifactRegistry",
                ))
